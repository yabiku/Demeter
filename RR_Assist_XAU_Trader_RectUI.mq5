//+------------------------------------------------------------------+
//|                      RR_Assist_XAU_Trader_RectUI.mq5             |
//|   XAUUSD Risk Reward Assist + Rect Main UI + Market/Pending EA   |
//+------------------------------------------------------------------+
#property strict
...
// 矩形UI用
input int    InpHandleBarsWidth        = 4;      // ハンドルの横幅
input int    InpHandleHeightPoints     = 70;     // ハンドルの高さ（points）
input bool   InpAllowZoneDrag          = true;   // Reward/Riskボックスのドラッグを有効化
...
// 極細補助線
string OBJ_ENTRY_LINE, OBJ_TP_LINE, OBJ_SL_LINE;

// メイン矩形
string OBJ_REWARD_RECT, OBJ_RISK_RECT;

// ハンドル矩形
string OBJ_ENTRY_HANDLE, OBJ_TP_HANDLE, OBJ_SL_HANDLE;
...
double NormalizePrice(const double price) { return NormalizeDouble(price, DigitsX()); }
...
string FmtPrice(const double price) { return DoubleToString(NormalizePrice(price), DigitsX()); }

string FmtLot(const double lot)
{
...
datetime HandleLeftTime()
{
   datetime t2 = RightTime();
   int sec = PeriodSeconds(_Period);
   if(sec <= 0) sec = 60;
   return (datetime)(t2 - sec * InpHandleBarsWidth);
}
...
double AbsD(const double v) { return (v >= 0.0 ? v : -v); }
...
void ApplyMode(const bool buyMode)
{
   double risk   = RiskPrice();
   double reward = RewardPrice();
...
bool PlaceMarketOrder(const bool is_buy)
{
   ...
   bool ok = false;
   if(is_buy)
      ok = trade.Buy(g_lot, _Symbol, 0.0, NormalizePrice(g_sl), NormalizePrice(g_tp), "RRRectUI_MarketBuy");
   else
      ok = trade.Sell(g_lot, _Symbol, 0.0, NormalizePrice(g_sl), NormalizePrice(g_tp), "RRRectUI_MarketSell");
...
bool PlacePendingOrder(const ENUM_ORDER_TYPE order_type)
{
   ...
   {
      case ORDER_TYPE_BUY_LIMIT:
         ok = trade.BuyLimit(g_lot, NormalizePrice(g_entry), _Symbol, NormalizePrice(g_sl), NormalizePrice(g_tp), ORDER_TIME_GTC, 0, "RRRectUI_BuyLimit");
         break;
      case ORDER_TYPE_SELL_LIMIT:
         ok = trade.SellLimit(g_lot, NormalizePrice(g_entry), _Symbol, NormalizePrice(g_sl), NormalizePrice(g_tp), ORDER_TIME_GTC, 0, "RRRectUI_SellLimit");
         break;
      case ORDER_TYPE_BUY_STOP:
         ok = trade.BuyStop(g_lot, NormalizePrice(g_entry), _Symbol, NormalizePrice(g_sl), NormalizePrice(g_tp), ORDER_TIME_GTC, 0, "RRRectUI_BuyStop");
         break;
      case ORDER_TYPE_SELL_STOP:
         ok = trade.SellStop(g_lot, NormalizePrice(g_entry), _Symbol, NormalizePrice(g_sl), NormalizePrice(g_tp), ORDER_TIME_GTC, 0, "RRRectUI_SellStop");
         break;
   }

   if(ok)
   {
      SetStatus("予約注文成功: " + EnumToString(order_type), clrLimeGreen);
      return true;
   }
...
bool CreateHLine(const string name, const double price, const color clr, const ENUM_LINE_STYLE style, const int width)
{
   SafeDelete(name);
...
bool CreateChartRect(const string name,
                     const datetime t1, const double p1,
                     const datetime t2, const double p2,
                     const color clr, const bool selectable,
                     const bool back, const int width=1)
{
   if(!Exists(name))
   {
...
bool CreatePriceTag(const string name, const string text, const int x, const int y, const color bg, const color txt)
{
   if(!Exists(name))
   {
...
//----------------------------------------------------
// Rect-main UI helpers
//----------------------------------------------------
double RectCenterPrice(const string obj)
{
   double p1 = ObjectGetDouble(0, obj, OBJPROP_PRICE, 0);
   double p2 = ObjectGetDouble(0, obj, OBJPROP_PRICE, 1);
   return NormalizePrice((p1 + p2) / 2.0);
}

void BuildMainRectsAndHandles()
{
   datetime tLeft   = iTime(_Symbol, _Period, 0);
   datetime tRight  = RightTime();
   datetime hLeft   = HandleLeftTime();
   datetime hRight  = RightTime();

   double rewardTop = MathMax(g_entry, g_tp);
   double rewardBot = MathMin(g_entry, g_tp);
   double riskTop   = MathMax(g_entry, g_sl);
   double riskBot   = MathMin(g_entry, g_sl);

   double hh = InpHandleHeightPoints * PointX();
   if(hh <= 0.0) hh = 50 * PointX();

   // メイン矩形
   CreateChartRect(OBJ_REWARD_RECT, tLeft, rewardTop, tRight, rewardBot, InpRewardFillColor, InpAllowZoneDrag, false, 1);
   CreateChartRect(OBJ_RISK_RECT,   tLeft, riskTop,   tRight, riskBot,   InpRiskFillColor,   InpAllowZoneDrag, false, 1);

   // ハンドル矩形（右側に小さく）
   CreateChartRect(OBJ_ENTRY_HANDLE, hLeft, g_entry + hh/2.0, hRight, g_entry - hh/2.0, InpEntryColor, true, false, 1);
   CreateChartRect(OBJ_TP_HANDLE,    hLeft, g_tp    + hh/2.0, hRight, g_tp    - hh/2.0, InpTPColor,    true, false, 1);
   CreateChartRect(OBJ_SL_HANDLE,    hLeft, g_sl    + hh/2.0, hRight, g_sl    - hh/2.0, InpSLColor,    true, false, 1);

   // 補助線（極細・点線）
   if(!Exists(OBJ_ENTRY_LINE)) CreateHLine(OBJ_ENTRY_LINE, g_entry, InpEntryColor, STYLE_DOT, 1);
   if(!Exists(OBJ_TP_LINE))    CreateHLine(OBJ_TP_LINE,    g_tp,    InpTPColor,    STYLE_DOT, 1);
   if(!Exists(OBJ_SL_LINE))    CreateHLine(OBJ_SL_LINE,    g_sl,    InpSLColor,    STYLE_DOT, 1);
...
void ReadPricesFromHandle(const string obj)
{
   double center = RectCenterPrice(obj);

   if(obj == OBJ_ENTRY_HANDLE)
   {
      if(InpKeepTPLSDistance)
      {
         double delta = center - g_entry;
         g_entry = center;
   ...
   else if(obj == OBJ_TP_HANDLE)
   {
      g_tp = center;
   }
   else if(obj == OBJ_SL_HANDLE)
   {
      g_sl = center;
   }
...
void ReadPricesFromZone(const string obj)
{
   double p1 = ObjectGetDouble(0, obj, OBJPROP_PRICE, 0);
   double p2 = ObjectGetDouble(0, obj, OBJPROP_PRICE, 1);
   ...
   if(obj == OBJ_REWARD_RECT)
   {
      if(g_is_buy)
      {
         g_tp    = NormalizePrice(top);
         g_entry = NormalizePrice(bot);
      }
      else
      {
         g_entry = NormalizePrice(top);
         g_tp    = NormalizePrice(bot);
      }
   }
   else if(obj == OBJ_RISK_RECT)
   {
      if(g_is_buy)
      {
         g_entry = NormalizePrice(top);
         g_sl    = NormalizePrice(bot);
      }
      else
      {
         g_sl    = NormalizePrice(top);
         g_entry = NormalizePrice(bot);
      }
   }
...
void BuildPanel()
{
   ...
   CreateLabel(OBJ_TITLE,  "RR ASSIST XAU RECT UI", InpPanelX + 12, InpPanelY + 7, 11, clrWhite, "Arial Bold");
   CreateLabel(OBJ_MODE,   "", InpPanelX + 12, InpPanelY + 36, 10, InpPanelTextColor);
   CreateLabel(OBJ_INFO,   "Rect main UI / drag handles / market & pending", InpPanelX + 12, InpPanelY + 54, 9, clrSilver);
...
void UpdatePanel()
{
   ...
   ObjectSetString(0, OBJ_VAL_RISK_MONEY,   OBJPROP_TEXT, DoubleToString(RiskMoney(), 2) + " " + ccy);
   ObjectSetString(0, OBJ_VAL_REWARD_MONEY, OBJPROP_TEXT, DoubleToString(RewardMoney(), 2) + " " + ccy);
   ObjectSetString(0, OBJ_VAL_BALANCE_RISK, OBJPROP_TEXT, DoubleToString(BalanceRiskPercent(), 2) + " %");
}
...
void DeleteAll()
{
   string names[] =
   {
      ...
      OBJ_ENTRY_LINE, OBJ_TP_LINE, OBJ_SL_LINE,
      OBJ_REWARD_RECT, OBJ_RISK_RECT,
      OBJ_ENTRY_HANDLE, OBJ_TP_HANDLE, OBJ_SL_HANDLE,
...
int OnInit()
{
   PREFIX = "RRRECT_" + IntegerToString((int)ChartID()) + "_";
   ...
   OBJ_ENTRY_HANDLE = PREFIX + "ENTRY_HANDLE";
   OBJ_TP_HANDLE    = PREFIX + "TP_HANDLE";
   OBJ_SL_HANDLE    = PREFIX + "SL_HANDLE";
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   {
      if(sparam == OBJ_ENTRY_HANDLE || sparam == OBJ_TP_HANDLE || sparam == OBJ_SL_HANDLE)
      {
         ReadPricesFromHandle(sparam);
         RefreshAll();
         return;
      }

      if(sparam == OBJ_REWARD_RECT || sparam == OBJ_RISK_RECT)
      {
         ReadPricesFromZone(sparam);
         RefreshAll();
         return;
      }
   }
