...
double MAX_fukumizon[MAGIC.Size()];
int MAX_Dansuu[MAGIC.Size()];
string max_buyorsell[MAGIC.Size()];
int SendMsgFlg[MAGIC.Size()];
...
public:
   ...
   {      
      ...
      // ダイアログ作成（背景の枠）
      if(!CAppDialog::Create(chart_id, EAName, 0, 20, 90, 255, 135+dy*(MAGIC.Size()-1)))
         return false;
...
void OnTick() {
   ...
   EnsurePositionsCache();      // 構成更新は必要時だけ g_positions_cache_dirty == trueの場合
   ...
   for(int i=1; i<(int)MAGIC.Size(); i++) {
      ...
      //MT5からメッセージ送信
      //含み損をSendNotificationで送信
      if(mqlNow.hour >=23 && mqlNow.min >= 59 && SendNotificationFlg && SendMsgFlg[i] == 1 && MAX_fukumizon[i] < 0) {
         MT5SendMessage(prefix[i] + ": MaxFukumizon=" + addcomma(MAX_fukumizon[i],0) + ": "+ (string)MAX_Dansuu[i]);
         MAX_fukumizon[i] = 0;
         MAX_Dansuu[i] = 0;
         SendMsgFlg[i] = 0;
      }
      if(mqlNow.hour == 0 && mqlNow.min >= 1 && SendMsgFlg[i] == 0 ) SendMsgFlg[i] = 1;
...
void RefreshFloatingProfitOnly()
{
   ...
   string com_msg = "";
   for(int i=0; i<(int)MAGIC.Size(); i++) {
      if(BuyProfit[i] < MAX_fukumizon[i]) {
         MAX_fukumizon[i] = BuyProfit[i];
         MAX_Dansuu[i] = BuyCount[i];
         max_buyorsell[i] = "Buy";
      }
      if(SellProfit[i] < MAX_fukumizon[i]) {
         MAX_fukumizon[i] = SellProfit[i];
         MAX_Dansuu[i] = SellCount[i];
         max_buyorsell[i] = "Sell";
      }
      com_msg += prefix[i] +":" + max_buyorsell[i] + " 最大含み損=" + addcomma(MAX_fukumizon[i],0) + " 段数=" + (string)MAX_Dansuu[i] + "\n";
   }
   Comment(com_msg);
}
